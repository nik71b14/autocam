...
void sliceModel(int triangleCount, float zSpan, bool preview = false) {
    ...
    // glm::mat4 projection = glm::ortho(-0.5f, 0.5f, -0.5f, 0.5f, 0.0f, zSpan); // Last two terms referred to eye position, are distances from the eye to the near and far planes
...
void sliceModel4(int triangleCount, float zSpan, bool preview = false) {
    ...
    for (int i = 0; i < RESOLUTION; ++i) {
        ...
        glDispatchCompute(1, RESOLUTION, 1); // Launch "RESOLUTION" workgroups, each with 1x1x1 threads (see compute shader), i.e. one thread per row at the end
        glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT | GL_SHADER_STORAGE_BARRIER_BIT); // Wait for compute shader to finish writing
