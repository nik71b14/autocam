...
void main() {
    // Unified ray setup for both projection types
    vec2 uv = (gl_FragCoord.xy / vec2(screenResolution)) * 2.0 - 1.0;
    vec4 nearClip = vec4(uv, -1.0, 1.0);  // Near plane at clip Z = -1
    vec4 farClip  = vec4(uv,  1.0, 1.0);  // Far plane at clip Z = 1
...
uniform bool isOrthographic;     // Orthographic projection flag
...
void main() {
    ...
    if (isOrthographic) {
        vec4 world = invViewProj * vec4(uv, 0.0, 1.0);
        rayOrigin = world.xyz / world.w;
        rayDir = normalize(vec3(0.0, 0.0, -1.0));  // Assumes orthographic looks along -Z
    } else {
