...
// uniform float uAlpha;          // Overall transparency (0.0-1.0)
// uniform float uDensity;        // Density factor (higher = more opaque)
...
void main() {
    float uAlpha = 0.6;          // Overall transparency (0.0-1.0)
    float uDensity = 4.0;        // Density factor (higher = more opaque)
    ...
    if (tmin > tmax || tmax < 0.0) {
        discard;  // no intersection with volume bounding box
    }

    tmin = max(tmin, 0.0);
    vec3 entry = rayOrigin + rayDir * tmin;
    vec3 exit = rayOrigin + rayDir * tmax;

    float rayLength = distance(entry, exit);
    ...
    float absorptionFactor = uDensity;
    vec3 pos = entry;

    for (int i = 0; i < maxSteps && transmittance > 0.01; ++i) {
        ...
        if (all(greaterThanEqual(ipos, ivec3(0))) && all(lessThan(ipos, resolution))) {
            if (isInsideVoxel(ipos.x, ipos.y, ipos.z)) {
                ...
                vec3 lightColor1 = vec3(1.0, 0.95, 0.8);
                vec3 lightDir2 = normalize(vec3(-0.3, 0.5, -0.7));
                vec3 lightColor2 = vec3(0.6, 0.7, 1.0);
                
                float diff1 = max(dot(normal, lightDir1), 0.0);
                float diff2 = max(dot(normal, lightDir2), 0.0);
                float ambient = 0.1;
                
                vec3 sampleColor = color * (ambient + diff1 * lightColor1 + diff2 * lightColor2);
                
                // Absorption and accumulation
                float absorption = exp(-absorptionFactor * stepSize);
                float alpha = 1.0 - absorption;
                
                accumulatedColor.rgb += sampleColor * alpha * transmittance;
        ...
        pos += rayDir * stepSize;
        if (distance(pos, entry) > rayLength) break;
    }

    // Apply overall transparency
    accumulatedColor.a *= uAlpha;
    ...
    // Use background depth since we're transparent
    gl_FragDepth = gl_FragCoord.z;
}
*/


// THIS IS OK (WITHOUT TRANSPARENCY)
#version 460 core
