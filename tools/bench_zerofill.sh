...
upd(){ local k=$1 t=$2; [ -z "$t" ] && return
  if [ -z "${m[$k]}" ] || awk "BEGIN{exit !($t<${m[$k]})}"; then m[$k]=$t; fi; }

printf "%-13s %9s %9s %7s   %9s %9s %7s\n" "workload" "S2 fill" "S2 nofill" "gain" "S3 fill" "S3 nofill" "gain"
printf -- '-%.0s' {1..74}; echo
for w in $WL; do
  declare -A m; for k in a b c d; do m[$k]=""; done
  for i in $(seq 0 "$N"); do
...
echo
echo "carving netto (ms), min of $N interleaved runs; gain = (no-fill/fill - 1), negative = faster."
