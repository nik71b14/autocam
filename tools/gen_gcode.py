...
class GcodeBuilder:
    ...
    def retract(self):
        self.lines.append(f"G0 Z{self.safe:.1f}")

    def rapid(self, x, y):
        self.lines.append(f"G0 X{x:.1f} Y{y:.1f}")

    def plunge(self, z):
        self.lines.append(f"G1 Z{z:.1f} F{self.feed}")

    def cut(self, x, y, z):
        self.lines.append(f"G1 X{x:.1f} Y{y:.1f} Z{z:.1f} F{self.feed}")
...
def build(features, params, safe_z, feed, seed=None):
    g = GcodeBuilder(safe_z, feed)
    g.lines.append("%")
...
def main():
    ...
    ap.add_argument("--safe-z", type=float, default=700.0, help="safe/clearance Z (no cut)")
    ...
    text = build(features, params, args.safe_z, args.feed, seed=args.seed if args.randomize else None)
    with open(args.out, "w") as fh:
        fh.write(text)
    print(f"wrote {args.out}  ({text.count(chr(10))} lines, features={features}"
